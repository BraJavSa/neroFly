...
class BebopWebotsMLPSim(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'MLP cargado | window_size={self.window_size} | dt_model={self.model_dt:.4f}s')
        ...
        self.v_history = deque([np.zeros(4)] * self.window_size, maxlen=self.window_size)
    ...
    def _build_mlp_input(self, v_smooth_current: np.ndarray, u_actual: np.ndarray) -> np.ndarray:
        v_win = np.array(self.v_history)
        u_win = np.array(self.u_history)
        x_input = np.hstack((v_win.flatten(), u_win.flatten(), u_actual.flatten())).reshape(1, -1)
        return x_input

    def _mlp_derivative(self, v_smooth_current: np.ndarray, u_actual: np.ndarray) -> np.ndarray:
        x_input = self._build_mlp_input(v_smooth_current, u_actual)
        dvdt = self.f_mlp(x_input).full().flatten()
        return dvdt
    ...
    def timer_cb(self):
        ...
        if self.mode == 'TAKING_OFF':
            ...
            if not self.warmup_done:
                ...
                if self.warmup_steps >= self.window_size:
                    ...
                    self.get_logger().info(f'Warm-up completado ({self.window_size} pasos) — MLP activo')
            else:
        ...
        if self.drone_node:
            ...
            axis = [qx / den, qy / den, qz / den] if den > 1e-06 else [0, 0, 1]
            self.drone_node.getField('rotation').setSFRotation(axis + [angle])
            if self.mode != 'IDLE':
                self.prop_angle += 0.6
                for i, prop in enumerate(self.prop_nodes):
                    if prop:
                        prop.getField('rotation').setSFRotation([0, 0, 1, self.prop_angle * (1 if i % 2 == 0 else -1)])
        self._step_gimbal()
        if self.gimbal_node:
            ...
            g_axis = [gqx / g_den, gqy / g_den, gqz / g_den] if g_den > 1e-06 else [0, 1, 0]
